...
window.onscroll = function (ev) {
    var houyi = getele(".back");
    var divide = getele(".divide");
    var all = $(window).height();
    if (divide < all * 0.45 && divide > all * 0.40) {
        $('.back-right').addClass("back-animate");
    } else if ($('#touch').offset().top <= $(window).scrollTop() || $('.back').offset().top > $(window).scrollTop() + $(window).height()) {
        $('.back-right').removeClass("back-animate");
    }
};
